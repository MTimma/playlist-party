...
export const PlaylistStats = ({ lobbyId, onStartGame, allPlayersReady }: PlaylistStatsProps) => {
  ...
  const canStartGame = () => {
    ...
    // Check both playlist requirements AND that all players are ready
      ...
      <div className="playlist-status">
        {canStartGame() ? (
          ...
          <div className="status-waiting">
            ...
            <span>
              {!allPlayersReady && 'Not all players are ready. '}
              {playlistData.stats.totalSongs < 2 && `Need at least 2 songs (${2 - playlistData.stats.totalSongs} more needed). `}
              {playlistData.stats.playersWithSongs < 2 && `Need at least 2 players to add songs (${2 - playlistData.stats.playersWithSongs} more needed).`}
            </span>
          </div>
        )}
      </div>
